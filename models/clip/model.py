...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        #self.attn_last = Attention(d_model, n_head)
    ...
    def attention_last(self, x: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        att = self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask,average_attn_weights=False)
        return att[0], att[1]
...
class CLIP(nn.Module):
    ...
    def encode_image_modified(self, image):
        return self.visual.last_step(image.type(self.dtype))
